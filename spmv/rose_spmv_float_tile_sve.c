...
/* Create the source (x) vector */
  ...
  for (row = 0; row < nrows; row++) {
    ...
    float sum = 0.0;
    svbool_t __pg0 = svwhilelt_b32((unsigned long )0,(unsigned long )((ia[row + 1] - 1 < (_lt_var_idx + 2 - 1))?(ia[row + 1] - 1) : (_lt_var_idx + 2 - 1)));
    for (idx = _lt_var_idx; idx <= (((ia[row + 1] - 1 < (_lt_var_idx + 2 - 1))?(ia[row + 1] - 1) : (_lt_var_idx + 2 - 1))); idx += 1 * svcntw()) {
      ...
      __part0 = (__vec4);
      __pg0 = svwhilelt_b32((unsigned long )idx,(unsigned long )(((ia[row + 1] - 1 < (_lt_var_idx + 2 - 1))?(ia[row + 1] - 1) : (_lt_var_idx + 2 - 1))));
    }
    __pg0 = svptrue_b32();
