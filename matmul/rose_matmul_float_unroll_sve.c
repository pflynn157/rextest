...
void matmul_simd(float **A,float **B,float **C)
{
  ...
  for (i = 0; i < 1024; i++) {
    for (j = 0; j < 1024; j++) {
      ...
      temp = 0;
      svbool_t __pg0 = svwhilelt_b32((unsigned long )0,(unsigned long )1023);
      for (k = 0; k <= 1023; k += 4 * svcntw()) {
        float *__ptr1 = A[i];
