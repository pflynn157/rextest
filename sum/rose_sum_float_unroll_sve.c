...
float sum(float *X)
{
  ...
  int i;
  float result = 0;
  svbool_t __pg0 = svwhilelt_b32((unsigned long )0,(unsigned long )10239999);
  for (i = 0; i <= 10239999; i += 4 * svcntw()) {
    svfloat32_t __vec1 = svld1(__pg0,&X[i]);
    svfloat32_t __vec2 = svadd_f32_m(__pg0,__vec1,__part0);
    __part0 = (__vec2);
    svfloat32_t __vec3 = svld1(__pg0,&X[i + 1]);
    svfloat32_t __vec4 = svadd_f32_m(__pg0,__vec3,__part0);
    __part0 = (__vec4);
    svfloat32_t __vec5 = svld1(__pg0,&X[i + 2]);
    svfloat32_t __vec6 = svadd_f32_m(__pg0,__vec5,__part0);
    __part0 = (__vec6);
    svfloat32_t __vec7 = svld1(__pg0,&X[i + 3]);
    svfloat32_t __vec8 = svadd_f32_m(__pg0,__vec7,__part0);
    __part0 = (__vec8);
    __pg0 = svwhilelt_b32((unsigned long )i,(unsigned long )10239999);
  }
  __pg0 = svptrue_b32();
